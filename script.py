...
def iterate_over_photo_files(folder, dest_folder):
    ...
    for fn in os.listdir(folder):
        ...
        if os.path.isfile(file_path):
            try:
                src_dest_map.update(
                    sorter.src_dest_path_from_photo_creation_date(
                        Path(file_path), dest_folder))
...
parser = argparse.ArgumentParser(
    description='renames photo files by creation date')
parser.add_argument(
    '--photo_folder',
    required=True,
    type=Path,
...
try:
    for src, dest in iterate_over_photo_files(photo_folder,
                                              dest_folder).items():
        try:
